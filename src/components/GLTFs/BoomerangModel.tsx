...
export default function BoomerangModel({
  idx,
  isDroppedBoomerang = false,
  keepFlying = false,
  ...props
}) {
  console.log(
    "🌟🚨 ~ file: BoomerangModel.tsx ~ line 20 ~ isDroppedBoomerang",
    isDroppedBoomerang
  );
  const group = useRef();
  ...
  const { x2, y2, z2 } = { x2: -18.13, y2: 3, z2: 9 };
  ...
  console.log(
    "🌟🚨 ~ file: BoomerangModel.tsx ~ line 35 ~ keepFlying",
    keepFlying
  );
  console.log("🌟🚨 ~ file: BoomerangModel.tsx ~ line 20 ~ status", status);

  const [{ opacity }] = useSpring(
    {
      opacity: status === "held" && !isDroppedBoomerang && !keepFlying ? 0 : 1,
    },
    [status, isDroppedBoomerang, keepFlying]
  );

  return (
    <animated.mesh material-transparent={true} material-opacity={opacity}>
