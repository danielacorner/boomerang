    ...
    () => ({
      ...
      onCollide: (e) => {
        ...
        if (isCollisionWithBoomerang) {
          ...
          const createItem = (itemType: ITEM_TYPES) =>
            setDroppedItems((p) => [...p, newItem(itemType, position.current)]);

          if (shouldDropItems) {
            onceRef.current = true;
            setTheyDroppedItems(true);

            createItem(ITEM_TYPES.MONEY);

            const powerup = Math.random() > 1 - POWERUP_PROBABILITY;
            if (powerup) {
              createItem(ITEM_TYPES.POWERUP);
            }

            const rangeUp = Math.random() > 1 - RANGEUP_PROBABILITY;
            if (rangeUp) {
              createItem(ITEM_TYPES.RANGEUP);
            }

            const heart = Math.random() > 1 - HEART_PROBABILITY;
            if (heart) {
              createItem(ITEM_TYPES.HEART);
            }
            ...
            if (droppedBoomerang) {
              createItem(ITEM_TYPES.BOOMERANG);
            }
          }
        }
      },
...
function newItem(itemType: ITEM_TYPES, position: [number, number, number]) {
  return {
    type: itemType,
    position: [
